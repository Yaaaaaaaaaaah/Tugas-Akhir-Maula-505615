...
function parseFile(filePath) {
    ...
    // Find the header row (first row with >=3 recognized columns)
    let headerRow = -1;
...
function computeQoS(rows, hasReceivedAt, sendIntervalSec = 300) {
    ...
    if (rows.length >= 2) {
        const firstTime = new Date(rows[0].tlocal).getTime();
        const lastTime = new Date(rows[rows.length - 1].tlocal).getTime();
